...
declare global {
  interface Window {
    modbusAPI: {
      ...
      write: (params: { functionCode: number; address: number; values: number | number[] }) => Promise<void>
      subscribeRawLog: (cb: (log: ModbusRawLog) => void) => () => void
      unsubscribeRawLog: (cb: () => void) => void
    }
  }
}
...
const MAX_LOG_ENTRIES = 100 // Limit logs to prevent DOM sluggishness
...
const minDelay = async <T,>(promise: Promise<T>, ms = 200): Promise<T> => {
  // Reduced minimum delay from 1000ms to 200ms for snappier UI
...
interface RegisterBlockProps {
  ...
  index: number
  isSelected: boolean
  // Important: these must be stable callbacks (useCallback from parent)
...
const RegisterBlock = memo<RegisterBlockProps>(({
  ...
  // Wrappers to avoid creating new functions in JSX rendering
  const handleMouseDown = () => onSelectionStart(index)
  const handleMouseEnter = () => onSelectionEnter(index)
  const handleClick = () => { if (!isEditing && !isReadOnly) handleFocus() }
  ...
  return (
    <div
      ...
      style={{
        ...
        border: isSelected ? '1px solid #3b82f6' : isEditing ? '1px solid #2563eb' : '1px solid #e9ecef',
        borderRadius: '4px',
        padding: '4px 2px',
        display: 'flex',
        ...
        cursor: 'default',
        userSelect: 'none',
        // Optimization: Use transform for hover effects if needed, avoid heavy layout properties
      }}
    >
      <div style={{ fontSize: '10px', color: isSelected ? '#1e3a8a' : '#adb5bd', marginBottom: '2px', fontFamily: 'monospace' }}>
        {addressLabel}
      </div>
      <input
        value={displayVal}
  ...
  // STRICT PROPS COMPARISON
  // Only re-render if visual properties change. 
  // This is critical for preventing Grid re-renders during Log updates or unrelated state changes.
  return (
    ...
    // Functions (onEdit, etc.) are assumed stable due to useCallback in parent
  )
})
...
const ModbusDebugger: React.FC = () => {
  ...
  const [monitorData, setMonitorData] = useState<{ startAddr: number; values: number[] } | null>(null)
  const [dataFormat, setDataFormat] = useState<DataFormat>(initialConfig.dataFormat)
  
  // Selection State
  const [selection, setSelection] = useState<{ start: number; end: number } | null>(null)
  const [isSelecting, setIsSelecting] = useState(false)
  const isSelectingRef = useRef(false) // Ref to track selection status without triggering re-renders in callbacks
  ...
  // --- Auto Save ---
  useEffect(() => {
    const config: SavedConfig = { settings, standardFc, customFcValue, address, addrFormat, countParam, dataFormat, customFcMode, showLogs }
    localStorage.setItem(STORAGE_KEY, JSON.stringify(config))
  }, [settings, standardFc, customFcValue, address, addrFormat, countParam, dataFormat, customFcMode, showLogs])
  ...
  // Scroll logs
  useEffect(() => {
    if (showLogs && logListRef.current) logListRef.current.scrollTop = logListRef.current.scrollHeight
  }, [logs, showLogs])
  ...
  // 2. Selection Enter - Uses Ref to avoid dependency loop
  ...
  // 3. Edit Cell - Corrected Logic with dataFormat dependency
  // Note: changing dataFormat will recreate this function, which triggers RegisterBlock re-render.
  // This is correct behavior because changing format MUST re-render blocks anyway.
  const handleCellEdit = useCallback((addr: number, newVal: string) => {
    setMonitorData((prevData) => {
        if (!prevData) return null
        let num = NaN
        const s = newVal.trim()
        
        // Correct Parsing Logic (Restored from original)
  ...
  }, [dataFormat]) // Depends on dataFormat to parse correctly
  ...
  const handleRead = async (silent = false) => {
    ...
    try {
      ...
      const execute = async () => {
        const res = await window.modbusAPI.read({ functionCode: fcNum as any, address: addrNum, count })
        const numValues = res.map((v) => (typeof v === 'boolean' ? (v ? 1 : 0) : v))
        setSelection(null)
        setMonitorData({ startAddr: addrNum, values: numValues })
        if (!silent) addLog('SYS', `Read ${res.length} items from ${formatAddress(addrNum, addrFormat)}`)
      }
  ...
  const handleWrite = async () => {
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (idx < 0 || idx >= monitorData.values.length) throw new Error('Data missing in monitor range')
      ...
      const execute = async () => {
        ...
        await window.modbusAPI.write({ functionCode: writeFC as number, address: startAddr, values: count === 1 ? valuesToWrite[0] : valuesToWrite })
        addLog('SYS', `Write OK to ${formatAddress(startAddr, addrFormat)}`)
        
        // Auto Refresh
        if (connected) {
           const readFc = fcNum === 5 || fcNum === 15 ? 1 : 3
           const refreshRes = await window.modbusAPI.read({ functionCode: readFc as any, address: startAddr, count })
  ...
  const preventEnter = (e: React.KeyboardEvent) => { if (e.key === 'Enter') e.preventDefault() }

  // --- MEMOIZED GRID RENDER ---
  // This is the core performance fix. It isolates Grid rendering from Logs/Connection/etc.
  const gridContent = useMemo(() => {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fill, minmax(${colWidth}, 1fr))`, gap: '8px' }}>
        {monitorData.values.map((val, idx) => {
          ...
          return (
            <RegisterBlock
              key={currentAddr} // Stable key based on address
              index={idx}
              address={currentAddr}
              value={val}
  ...
  }, [monitorData, dataFormat, addrFormat, selection, handleSelectionStart, handleSelectionEnter, handleCellEdit])


  // --- Render ---
  const inputBase = { padding: '0 10px', height: '36px', borderRadius: '6px', border: '1px solid #e4e4e7', fontSize: '13px', color: '#18181b', background: '#fff', transition: 'border 0.2s', boxSizing: 'border-box' as const }
  const labelStyle = { fontSize: '12px', fontWeight: 600, color: '#495057', marginBottom: '6px', display: 'block' }
  const rowStyle = { display: 'flex', gap: '12px', alignItems: 'flex-end', flexWrap: 'wrap' as const }
  const flexFixed = (w: string) => ({ flex: `0 0 ${w}` })
  const flexGrow = { flex: '1 1 120px' }

  return (
    <div style={{ height: '100%', width: '100%', padding: '20px', boxSizing: 'border-box', background: '#fff', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      
      {/* 1. Settings */}
      <div style={rowStyle}>
        <div style={flexFixed('90px')}>
          <span style={labelStyle}>Mode</span>
          <select style={{ ...inputBase, width: '100%', padding: '0 8px' }} value={settings.mode} onChange={(e) => updateInfo('mode', e.target.value)}>
        ...
        <div style={flexFixed('80px')}>
          ...
          <input style={{ ...inputBase, width: '100%' }} type="number" value={settings.slaveId} onChange={(e) => updateInfo('slaveId', parseInt(e.target.value))} onKeyDown={preventEnter} />
        </div>
          ...
          {settings.mode === 'RTU' ? (
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <select style={{ ...inputBase, flex: 1, minWidth: '150px' }} value={settings.serialPort} onChange={(e) => updateInfo('serialPort', e.target.value)}>
                  {ports.map((p) => <option key={p.path} value={p.path}>{p.path}</option>)}
                </select>
                <button onClick={scanPorts} style={{ ...inputBase, width: '36px', background: '#f8f9fa', padding: 0, cursor: 'pointer' }}>↻</button>
                <select style={{ ...inputBase, width: '100px', flex: '0 0 auto' }} value={settings.baudRate} onChange={(e) => updateInfo('baudRate', parseInt(e.target.value))}>
             ...
             <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input style={{ ...inputBase, flex: 1 }} value={settings.ipAddress} onChange={(e) => updateInfo('ipAddress', e.target.value)} onKeyDown={preventEnter} />
                <input style={{ ...inputBase, width: '80px' }} type="number" value={settings.port} onChange={(e) => updateInfo('port', parseInt(e.target.value))} onKeyDown={preventEnter} />
              </div>
            </div>
          )}
        </div>
        <button onClick={handleConnect} disabled={sending} style={{ ...inputBase, width: '140px', fontWeight: 600, cursor: sending ? 'wait' : 'pointer', background: connected ? '#ef4444' : '#18181b', color: '#fff', border: 'none', opacity: sending ? 0.7 : 1 }}>
      ...
      <div style={rowStyle}>
         <div style={flexFixed('220px')}>
          ...
          <div style={{ display: 'flex' }}>
            {customFcMode ? (
              <input style={{ ...inputBase, width: '150px', borderRight: 'none', borderRadius: '6px 0 0 6px' }} value={customFcValue} onChange={(e) => setCustomFcValue(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCommand()} placeholder="FC (e.g. 0x06)" />
            ) : (
              <select style={{ ...inputBase, width: '150px', borderRight: 'none', borderRadius: '6px 0 0 6px' }} value={standardFc} onChange={(e) => setStandardFc(e.target.value)}>
            ...
            <button onClick={() => setCustomFcMode(!customFcMode)} style={{ ...inputBase, width: '70px', borderRadius: '0 6px 6px 0', background: '#f8f9fa', cursor: 'pointer', fontSize: '11px', fontWeight: 600, padding: 0 }}>
        ...
        <div style={flexGrow}>
          ...
          <div style={{ display: 'flex' }}>
            <input style={{ ...inputBase, width: '100%', borderRight: 'none', borderRadius: '6px 0 0 6px', fontFamily: 'monospace' }} value={address} onChange={(e) => setAddress(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleMainAction()} />
            <button onClick={toggleAddrFormat} style={{ ...inputBase, width: '50px', borderRadius: '0 6px 6px 0', background: '#f8f9fa', cursor: 'pointer', fontSize: '11px', fontWeight: 600, padding: 0 }}>{addrFormat}</button>
          </div>
        </div>
        <div style={flexFixed('100px')}>
          <span style={labelStyle}>Count</span>
          <input style={{ ...inputBase, width: '100%', fontFamily: 'monospace' }} value={countParam} onChange={(e) => setCountParam(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleMainAction()} />
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
          {(!(!customFcMode && [6,16].includes(parseFC(effectiveFc)))) && (
             <button onClick={() => setAutoRead(!autoRead)} disabled={!connected} style={{ ...inputBase, width: '80px', fontWeight: 600, border: 'none', cursor: !connected ? 'not-allowed' : 'pointer', background: autoRead ? '#10b981' : '#f8f9fa', color: autoRead ? '#fff' : '#18181b' }}>{autoRead ? 'Stop' : 'Auto'}</button>
          )}
          <button onClick={handleMainAction} disabled={!connected || sending} style={{ ...inputBase, width: '100px', fontWeight: 600, border: 'none', cursor: !connected || sending ? 'not-allowed' : 'pointer', background: '#2563eb', color: '#fff', opacity: !connected || sending ? 0.5 : 1 }}>
            {sending ? '...' : 'Exec'}
          </button>
        </div>
      </div>

      {/* 3. Data Monitor (Isolated Render) */}
      <div style={{ flex: showLogs ? 2 : 1, transition: 'flex 0.3s', display: 'flex', flexDirection: 'column', minHeight: '0', border: '1px solid #e4e4e7', borderRadius: '8px', background: '#fafafa', overflow: 'hidden' }}>
        <div style={{ padding: '8px 16px', background: '#fff', borderBottom: '1px solid #e4e4e7', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            {(['DEC_U', 'DEC_S', 'UINT32', 'HEX', 'FLOAT', 'ASCII'] as DataFormat[]).map((f) => (
              <button key={f} onClick={() => setDataFormat(f)} style={{ background: dataFormat === f ? '#e4e4e7' : 'transparent', color: dataFormat === f ? '#000' : '#71717a', border: 'none', padding: '4px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>{f}</button>
            ))}
          </div>
        </div>
      ...
      {/* 4. Logs */}
      <div style={{ flex: showLogs ? 1 : '0 0 32px', transition: 'flex 0.3s', border: '1px solid #e4e4e7', borderRadius: '8px', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: '#fff' }}>
        <div style={{ padding: '0 12px', height: '32px', background: '#f4f4f5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', borderBottom: showLogs ? '1px solid #e4e4e7' : 'none' }} onClick={() => setShowLogs(!showLogs)}>
           <span style={{ color: '#18181b', fontSize: '11px', fontWeight: 700 }}>LOGS {logs.length > 0 && !showLogs && `— ${logs[logs.length-1].msg}`}</span>
           <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }} onClick={e => e.stopPropagation()}>
             {showLogs && <button onClick={() => setLogs([])} style={{ background: 'transparent', border: 'none', fontSize: '11px', cursor: 'pointer', color: '#71717a' }}>Clear</button>}
             <span style={{ fontSize: '10px', color: '#a1a1aa' }}>{showLogs ? '▼' : '▲'}</span>
           </div>
        </div>
        <div ref={logListRef} style={{ flex: 1, overflowY: 'auto', padding: '8px 12px', fontFamily: 'monospace', fontSize: '12px' }}>
          {logs.map(l => (
             <div key={l.id} style={{ marginBottom: '4px', display: 'flex', gap: '8px' }}>
               <span style={{ color: '#a1a1aa', minWidth: '60px' }}>{l.time}</span>
               <span style={{ fontWeight: 700, minWidth: '24px', color: l.dir === 'TX' ? '#d97706' : l.dir === 'RX' ? '#059669' : '#ef4444' }}>{l.dir}</span>
